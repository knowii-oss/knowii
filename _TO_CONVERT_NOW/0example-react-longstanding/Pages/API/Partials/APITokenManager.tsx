...
export default function APITokenManager({ tokens, availablePermissions, defaultPermissions }: Props) {
  ...
  return (
    <div>
                      ...
                      onChange={(e) => {
                        if (createApiTokenForm.data.permissions.includes(e.currentTarget.value)) {
                          createApiTokenForm.setData(
                            'permissions',
                            createApiTokenForm.data.permissions.filter((p) => p !== e.currentTarget.value),
                          );
                        } else {
                ...
                {tokens.map((token) => (
                  <div className="flex items-center justify-between" key={token.id}>
                    ...
                    <div className="flex items-center">
                      ...
                      <PrimaryButton className="cursor-pointer ml-6 text-sm text-red-500" onClick={() => confirmApiTokenDeletion(token)}>
                        Delete
                      </PrimaryButton>
                    </div>
                  </div>
      ...
      {/* <!-- Token Value Modal --> */}
      <DialogModal isOpen={displayingToken} onClose={() => setDisplayingToken(false)}>
        <DialogModal.Content title={'API Token'}>
          <div>Please copy your new API token. For your security, it won't be shown again.</div>

          <div className="mt-4 bg-gray-100 px-4 py-2 rounded font-mono text-sm text-gray-500">{page.props?.jetstream?.flash?.token}</div>
        </DialogModal.Content>
        <DialogModal.Footer>
      ...
      <DialogModal isOpen={!!managingPermissionsFor} onClose={() => setManagingPermissionsFor(null)}>
                    ...
                    onChange={(e) => {
                      if (updateApiTokenForm.data.permissions.includes(e.currentTarget.value)) {
                        updateApiTokenForm.setData(
                          'permissions',
                          updateApiTokenForm.data.permissions.filter((p) => p !== e.currentTarget.value),
                        );
                      } else {
      ...
      <ConfirmationModal isOpen={!!apiTokenBeingDeleted} onClose={() => setApiTokenBeingDeleted(null)}>
