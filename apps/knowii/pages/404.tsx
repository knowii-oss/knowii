...
export const getStaticProps: GetStaticProps = async (ctx) => {
  ...
  const translations = await serverSideTranslations(locale, [I18N_TRANSLATIONS_COMMON], i18nConfig);
  return {
    props: {
      ...translations,
    },
  ...
  return (
    ...
    >
      <div className="flex flex-col text-center px-8 py-36 bg-primary-500 dark:bg-steel-500 gap-2">
        <h1 className="text-white">{t('pageNotFound.title')}</h1>
        <h2 className="text-white">{t('pageNotFound.message')}</h2>
        <Link href="/" className="site-button">
          {t('pageNotFound.goBackHomeLink')}
        </Link>
      </div>
    </Layout>
  );
